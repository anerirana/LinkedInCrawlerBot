...
class Location:
    def __init__(self, location: str):
        ...
        if ',' in location:
            try:
                self.city = location.split(',')[-2].strip()
...
class Job:
    def __init__(self, position: str, company: str, location: Location, date_range: str):
...
class Profile:
    def __init__(self, url: str, name: str, location: Location, skills: [str], job: Job):
        self.url = url
        self.name = name
        self.location = location
        self.skills = skills
    ...
    def reprJSON(self):
        return dict(url=self.url, name=self.name, location=self.location, skills=self.skills, jobs=self.jobs)
